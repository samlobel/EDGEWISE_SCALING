...
with tf.sg_context(name='discriminator', size=4, stride=2, act='leaky_relu', batch_size=disc_batch_size):
    disc = (xx.sg_conv_with_scale(dim=64)
            ...
            .sg_upconv_with_scale(dim=64)
            .sg_upconv_with_scale(dim=1, act='linear'))
...
max_keep=500
# do training
alt_train(log_interval=10, save_interval=save_interval, max_keep=max_keep, max_ep=30, ep_size=data.train.num_batch, early_stop=False)
